...
var (
	...
	libvirtDomainTimedOutDesc = prometheus.NewDesc(
		...
		[]string{"domain", "project_id", "instance_id", "instance_name"},
		nil)
	...
	libvirtDomainState = prometheus.NewDesc(
		...
		[]string{"domain", "project_id", "state_desc", "instance_id", "instance_name"},
		nil)
	libvirtDomainInfoMaxMemDesc = prometheus.NewDesc(
		...
		[]string{"domain", "project_id", "instance_id", "instance_name"},
		nil)
	libvirtDomainInfoMaxMemGigabytesDesc = prometheus.NewDesc(
		prometheus.BuildFQName(namespace, "domain_info", "maximum_memory_gigabytes"),
		"Maximum allowed memory of the domain, in gigabytes.",
		[]string{"domain", "project_id", "instance_id", "instance_name"},
		nil)
	...
	libvirtDomainBlockStatsInfo = prometheus.NewDesc(
		...
		[]string{"domain", "project_id", "instance_id", "instance_name", "disk_type", "target_bus", "driver_name", "driver_type", "driver_cache", "driver_discard", "source_file", "source_protocol", "target_device", "serial"},
		nil)
	...
	libvirtDomainBlockCapacityBytesDesc = prometheus.NewDesc(
		...
		[]string{"domain", "project_id", "instance_id", "instance_name", "target_device"},
		nil)
	libvirtDomainBlockTotalCapacityGigabytesDesc = prometheus.NewDesc(
		prometheus.BuildFQName(namespace, "domain_block_stats", "total_capacity_gigabytes"),
		"Total logical size in gigabytes of all block devices for the domain.",
		[]string{"domain", "project_id", "instance_id", "instance_name"},
		nil)
	...
	libvirtDomainInterfaceInfo = prometheus.NewDesc(
		...
		[]string{"domain", "project_id", "instance_id", "instance_name", "interface_type", "source_bridge", "target_device", "mac_address", "model_type", "mtu_size"},
		nil)
	...
	libvirtDomainVCPUStatsState = prometheus.NewDesc(
		...
		"State of the vCPU.",
		[]string{"domain", "project_id", "instance_id", "instance_name", "vcpu"},
		nil)
	...
	libvirtDomainInfoDesc = prometheus.NewDesc(
		...
		[]string{"domain", "project_id", "instance_id", "instance_name", "os_type", "os_type_arch", "os_type_machine"},
		nil)
	...
	// optimization metrics: hostname included, project_id excluded
	libvirtOptimizationDomainInfoCpuTimeDesc = prometheus.NewDesc(
		prometheus.BuildFQName(namespace, "optimization_domain_info", "cpu_time_seconds_total"),
		"Amount of CPU time used by the domain, in seconds.",
		[]string{"hostname", "domain", "instance_id", "instance_name"},
		nil)
	libvirtOptimizationDomainInfoNrVirtCpuDesc = prometheus.NewDesc(
		prometheus.BuildFQName(namespace, "optimization_domain_info", "virtual_cpus"),
		"Number of virtual CPUs for the domain.",
		[]string{"hostname", "domain", "instance_id", "instance_name"},
		nil)
	libvirtOptimizationDomainMemoryStatsRssBytesDesc = prometheus.NewDesc(
		prometheus.BuildFQName(namespace, "optimization_domain_memory_stats", "rss_bytes"),
		"Resident Set Size of the process running the domain.",
		[]string{"hostname", "domain", "instance_id", "instance_name"},
		nil)
...
func CollectFromLibvirt(ch chan<- prometheus.Metric, uri string, driver libvirt.ConnectURI, logger *slog.Logger, timeout time.Duration, maxConcurrentCollects int) (err error) {
	...
	if osErr != nil {
		logger.Error("failed to get hostname", "msg", osErr)
		hostname = "unknown"
	}
	...
	// try to get hostname from libvirt, fallback to os hostname
	if lvHostname, err := l.ConnectGetHostname(); err == nil && lvHostname != "" {
		hostname = lvHostname
	}
	...
	worker := func() {
		...
		for {
			...
			case domain := <-domainChan:
				if err, hasTimedOut := CollectDomain(ch, l, domain, hostname, logger, timeout); err != nil {
					logger.Error("failed to collect domain", "domain", domain.domainName, "msg", err)
					if hasTimedOut {
						ch <- prometheus.MustNewConstMetric(
							libvirtDomainTimedOutDesc,
							prometheus.GaugeValue,
							float64(1),
							domain.domainName, domain.projectId, domain.instanceId, domain.instanceName)
			...
			case pool := <-poolChan:
				if err, hasTimedOut := CollectStoragePoolInfo(ch, l, pool, hostname, logger, timeout); err != nil {
...
func CollectDomain(ch chan<- prometheus.Metric, l *libvirt.Libvirt, domain domainMeta, hostname string, logger *slog.Logger, timeout time.Duration) (err error, hasTimedOut bool) {
	var (
		rState                     uint8
		rvirCpu                    uint16
	...
	ch <- prometheus.MustNewConstMetric(libvirtDomainInfoMaxMemGigabytesDesc, prometheus.GaugeValue, float64(rmaxmem)/float64(1024*1024), promLabels...)
	...
	ch <- prometheus.MustNewConstMetric(libvirtOptimizationDomainInfoNrVirtCpuDesc, prometheus.GaugeValue, float64(rvirCpu), optimizationLabels...)
	ch <- prometheus.MustNewConstMetric(libvirtOptimizationDomainInfoCpuTimeDesc, prometheus.CounterValue, float64(rcputime)/1e9, optimizationLabels...)
...
func CollectAdditionalDomainBlockDeviceInfo(ch chan<- prometheus.Metric, l *libvirt.Libvirt, domain domainMeta, promLabels []string, logger *slog.Logger, timeout time.Duration) (err error, hasTimedOut bool) {
	...
	ch <- prometheus.MustNewConstMetric(libvirtDomainBlockTotalCapacityGigabytesDesc, prometheus.GaugeValue, float64(totalCapacityBytes)/float64(1024*1024*1024), promLabels...)
	...
	for _, param := range domainStatsMemory.Params {
		...
		case "balloon.hugetlb_pgfail":
			ch <- prometheus.MustNewConstMetric(
				...
				promLabels...)
		case "balloon.rss":
			rssBytes := float64(param.Value.I.(uint64) * 1024)
			ch <- prometheus.MustNewConstMetric(
				...
				promLabels...)
			ch <- prometheus.MustNewConstMetric(
				libvirtOptimizationDomainMemoryStatsRssBytesDesc,
				prometheus.GaugeValue,
				rssBytes,
				domain.hostname, domain.domainName, domain.instanceId, domain.instanceName)
		}
	}
	ch <- prometheus.MustNewConstMetric(
...
func CollectStoragePoolInfo(ch chan<- prometheus.Metric, l *libvirt.Libvirt, pool libvirt.StoragePool, hostname string, logger *slog.Logger, timeout time.Duration) (err error, hasTimedOut bool) {
...
func (e *LibvirtExporter) Describe(ch chan<- *prometheus.Desc) {
	...
	ch <- libvirtDomainInfoMaxMemGigabytesDesc
	...
	ch <- libvirtDomainBlockTotalCapacityGigabytesDesc
	...
	ch <- libvirtOptimizationDomainInfoCpuTimeDesc
	ch <- libvirtOptimizationDomainInfoNrVirtCpuDesc
	ch <- libvirtOptimizationDomainMemoryStatsRssBytesDesc
