...
def load_definitions(conf, defaults, config_file, fallback_file, excluded_events_file):
    ...
    if not os.path.exists(excluded_events_file):
        LOG.info("Excluded events file " + excluded_events_file + " does not exist.")
        excluded_events_file = None
    ...
    if excluded_events_file is not None:
        with open(excluded_events_file) as ef:
            exc_file = ef.read()
        try:
            excluded_events = yaml.safe_load(exc_file)
        except yaml.YAMLError as err:
            LOG.warning("Invalid syntax in excluded events file. Skipping...")
            excluded_events = None

    LOG.debug("Definitions: %s", definition_cfg)
    return definition_cfg, excluded_events
