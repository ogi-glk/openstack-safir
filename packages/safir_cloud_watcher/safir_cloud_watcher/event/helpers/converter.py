...
from safir_cloud_watcher.event.helpers import declarative, models
...
class NotificationEventsConverter(object):
    ...
    def __init__(self, conf, trait_plugin_mgr):
        self.conf = conf

        def_file = os.path.join(BASE_DIR, '../data/event_definitions.yaml')
        exc_file = os.path.join(BASE_DIR, '../data/excluded_events.yaml')
        events_config, self.excluded_events = declarative.load_definitions(
            conf, [], conf.notification.definitions_cfg_file,
            def_file, exc_file)

        raw_levels = [level.lower() for level in self.conf.notification.store_raw]
        self.definitions = [
            EventDefinition(event_def, trait_plugin_mgr, raw_levels)
            for event_def in reversed(events_config)]

        add_catchall = not self.conf.notification.drop_unmatched_notifications
