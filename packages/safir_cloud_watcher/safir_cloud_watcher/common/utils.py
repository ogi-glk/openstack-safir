...
LICENCE_STATUS = enum(
    ...
    DUE_DATE_EXCEEDED="DUE_DATE_EXCEEDED",
    LICENCE_VALID="LICENCE_VALID"
)

OPERATION_TYPE = enum(
    DISABLE_SOME_HYPERVISORS='DISABLE_SOME_HYPERVISORS',
    DISABLE_ALL_HYPERVISORS='DISABLE_ALL_HYPERVISORS',
    NONE="NONE"
)
...
def ts2utc(ts):
    """Returns a datetime for the given timestamp."""
    try:
...
def get_day_of_week():
    return timeutils.utcnow().weekday()
...
def check_time_state(timestamp=None, period=0, wait_time=0):
    # If timestamp is None defined start timestamp from current mounth
    if not timestamp:
